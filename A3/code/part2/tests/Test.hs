...
--------------------------------------------------------------------pNum--------------------------------------------------------------------
  ...
  testCase "pNum2" $ parseString "5593" @?= Right [SExp (Const (IntVal 5593))],
  testCase "pNum3" $ parseString "-5" @?= Right [SExp (Const (IntVal (-5)))],
  testCase "pNum4" $ parseString "-554328" @?= Right [SExp (Const (IntVal (-554328)))],
  testCase "pNum5" $ parseString "0" @?= Right [SExp (Const (IntVal 0))],
  testCase "pNum6" $ parseString "-0" @?= Right [SExp (Const (IntVal 0))],
...
--------------------------------------------------------------------pIdent--------------------------------------------------------------------
  ...
  testCase "pIdent8" $ parseString "_4var1_" @?= Right [SExp (Var ("_4var1_"))],
...
--------------------------------------------------------------------pExprOpt--------------------------------------------------------------------
  ...
  testCase "ExprOpt9" $ parseString "x not in 5" @?= Right [SExp (Not (Oper In (Var "x") (Const (IntVal 5))))],
...
--------------------------------------------------------------------Not--------------------------------------------------------------------
  ...
  testCase "NUUT NUUT" $ parseString "not not x + 5" @?= Right [SExp (Not (Not (Oper Plus (Var "x") (Const (IntVal 5)))))],
--------------------------------------------------------------------TermOpt--------------------------------------------------------------------
...
--------------------------------------------------------------------pString--------------------------------------------------------------------
  ...
  testCase "pString7" $ parseString "''" @?= Right [SExp (Const (StringVal ("")))],
...
--------------------------------------------------------------------Compr--------------------------------------------------------------------
  ...
  testCase "Compr1" $ parseString "[x for x in x]" @?= Right [SExp (Compr (Var ("x")) [CCFor "x" (Var "x")])],
  testCase "Compr5" $ parseString "[x for x in x if u]" @?= Right [SExp (Compr (Var ("x")) [CCFor "x" (Var "x"), CCIf (Var "u")])],
  testCase "Compr6" $ parseString "[x for x in x in u]" @?= Right [SExp (Compr (Var "x") [CCFor "x" (Oper In (Var "x") (Var "u"))])],
  testCase "Compr7" $ parseString "[x+2 for x in x for y in 7 if 1 < 2]" @?= Right [SExp (Compr (Oper Plus (Var "x") (Const (IntVal 2))) [CCFor "x" (Var "x"),CCFor "y" (Const (IntVal 7)),CCIf (Oper Less (Const (IntVal 1)) (Const (IntVal 2)))])],
...
--------------------------------------------------------------------pExprs--------------------------------------------------------------------
  ...
  testCase "pExprs1" $ parseString "5; 4" @?= Right [SExp (Const (IntVal 5)), SExp (Const (IntVal 4))],
  testCase "pExprs2" $ parseString "5 + x; 4" @?= Right [SExp (Oper Plus (Const (IntVal 5)) (Var "x")),SExp (Const (IntVal 4))],
  testCase "pExprs3" $ parseString "'This is a string'; 4" @?= Right [SExp (Const (StringVal "This is a string")),SExp (Const (IntVal 4))],
  testCase "pExprs4" $ parseString "'This is a string'; 4; [x for x in x]" @?= Right [SExp (Const (StringVal "This is a string")),SExp (Const (IntVal 4)), SExp (Compr (Var ("x")) [CCFor "x" (Var "x")])],
  testCase "pExprs5" $ parseString "'This is a string'; 4 % 8; [x for x in x]" @?= Right [SExp (Const (StringVal "This is a string")),SExp (Oper Mod (Const (IntVal 4)) (Const (IntVal 8))),SExp (Compr (Var "x") [CCFor "x" (Var "x")])],
  testCase "pExprs6" $ parseString "x=5; x+2" @?= Right [SDef "x" (Const (IntVal 5)),SExp (Oper Plus (Var "x") (Const (IntVal 2)))],
  testCase "pExprs7" $ parseString "x=5; y=7; x+y" @?= Right [SDef "x" (Const (IntVal 5)),SDef "y" (Const (IntVal 7)),SExp (Oper Plus (Var "x") (Var "y"))],
  testCase "pExprs8" $ parseString "x=5; y=7; [x for x in y if 1 < 2]" @?= Right [SDef "x" (Const (IntVal 5)),SDef "y" (Const (IntVal 7)),SExp (Compr (Var "x") [CCFor "x" (Var "y"),CCIf (Oper Less (Const (IntVal 1)) (Const (IntVal 2)))])],
  testCase "pExprs9" $ parseString "x = 5; y = 7; [x for x in y if 1 < 2]" @?= Right [SDef "x" (Const (IntVal 5)),SDef "y" (Const (IntVal 7)),SExp (Compr (Var "x") [CCFor "x" (Var "y"),CCIf (Oper Less (Const (IntVal 1)) (Const (IntVal 2)))])],
  testCase "pExprs10" $ parseString "x = 5; y = 7*x; [x for x in y if 1 < 2]" @?= Right [SDef "x" (Const (IntVal 5)),SDef "y" (Oper Times (Const (IntVal 7)) (Var "x")),SExp (Compr (Var "x") [CCFor "x" (Var "y"),CCIf (Oper Less (Const (IntVal 1)) (Const (IntVal 2)))])],
  testCase "pExprs11" $ parseString "x = 5; y = 7; [x      for x    in y     if 1  < 2    ]" @?= Right [SDef "x" (Const (IntVal 5)),SDef "y" (Const (IntVal 7)),SExp (Compr (Var "x") [CCFor "x" (Var "y"),CCIf (Oper Less (Const (IntVal 1)) (Const (IntVal 2)))])],


--------------------------------------------------------------------pIdent pExprs--------------------------------------------------------------------
  testCase "ident (Exp)" $ parseString "range (5)" @?= Right [SExp (Call "range" [Const (IntVal 5)])],
  testCase "ident (Exp)" $ parseString "range (5+y)" @?= Right [SExp (Call "range" [Oper Plus (Const (IntVal 5)) (Var "y")])],
  testCase "ident (Exp)" $ parseString "print (5)" @?= Right [SExp (Call "print" [Const (IntVal 5)])],
  testCase "ident (Exp)" $ parseString "print (5 + 6)" @?= Right [SExp (Call "print" [Oper Plus (Const (IntVal 5)) (Const (IntVal 6))])],
  testCase "ident (Exp)" $ parseString "print ('This is a string')" @?= Right [SExp (Call "print" [Const (StringVal "This is a string")])],
  testCase "ident (Exp)" $ parseString "print (x == Y)" @?= Right [SExp (Call "print" [Oper Eq (Var "x") (Var "Y")])],
